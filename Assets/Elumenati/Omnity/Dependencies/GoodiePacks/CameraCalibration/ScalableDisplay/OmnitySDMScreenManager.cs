...
public class OmnitySDMScreenManager : OmnityTabbedFeature {
    static public OmnityPluginsIDs _myOmnityPluginsID = OmnityPluginsIDs.OmnitySDMScreenManager;
    override public OmnityPluginsIDs myOmnityPluginsID { get { return _myOmnityPluginsID; } }

    static public OmnitySDMScreenManager OmniCreateSingleton(GameObject go) {
        return GetSingleton<OmnitySDMScreenManager>(ref singleton, go);
    }

    public static OmnitySDMScreenManager singleton;

    static public OmnitySDMScreenManager Get() {
        return singleton;
    }
    ...
    private void Awake() {
        if (VerifySingleton<OmnitySDMScreenManager>(ref singleton)) {
            Omnity.onReloadEndCallbackPriority.AddLoaderFunction(PriorityEventHandler.Ordering.OmnitySDMScreenManager, CoroutineLoader);
    ...
    public override void WriteXMLDelegate(XmlTextWriter xmlWriter) {
        ...
        xmlWriter.WriteElementString("dualPipeStereo", dualPipeStereo.ToString());
        xmlWriter.WriteEndElement();
    }
    ...
    public override void ReadXMLDelegate(XPathNavigator nav) {
        ...
        dualPipeStereo = OmnityHelperFunctions.ReadElementBoolDefault(nav, ".//dualPipeStereo", false);
    }
    ...
    public bool dualPipeStereo = false;
    ...
    private System.Collections.IEnumerator CoLoad(Omnity myOmnity) {
        ...
        yield return StartCoroutine(CoRoutineLoadFileSetOthographicPerspective(ps, rcLayerMask, screenLayer, fpcLayerMask, "mono", dualPipeStereo));
    ...
    public override void MyGuiCallback(Omnity anOmnity) {
        if (!anOmnity.pluginIDs.Contains((int)OmnityPluginsIDs.OmnitySDMScreenManager)) {
            return;
        }
        Tabs(anOmnity);
        if (OmnityHelperFunctions.BoolInputResetWasChanged("Dual Pipe Stereo", ref dualPipeStereo, false)) {
        }
        OmnityHelperFunctions.ExpanderSimple("BlendGamma", "BlendGamma", "BlendGamma", () => {
        ...
        for (int i = 0; i < screenMapperData.Count; i++) {
            string channeltitle = "Channel " + i;
            OmnityHelperFunctions.ExpanderSimple(channeltitle, channeltitle, channeltitle, () => {
                ...
                screenMapperData[i].yMin = OmnityHelperFunctions.FloatInputReset("yMin (v)", screenMapperData[i].yMin, -90);
            });
        }
    ...
    public void ReconnectTextures(Omnity anOmnity) {
        ...
        if (!anOmnity.PluginEnabled(OmnityPluginsIDs.OmnitySDMScreenManager)) {
            return;
        }
    ...
    public IEnumerator CoRoutineLoadFileSetOthographicPerspective(SDMPROJECTOR[] files, int rcLayerMask, int screenLayer, int fpcLayerMask, string prefix, bool stereo) {
        ...
        myOmnity.FinalPassCameraArrayResize(startLengthSS + files.Length * eyeCount, false);
...
#pragma warning restore CS0618  
            ...
            for (int eye = 0; eye < eyeCount; eye++) {
                ...
                FinalPassSetup.ApplySettings(FinalPassShaderType.EdgeBlendVertexColorPerspective, screenShape.renderer);
